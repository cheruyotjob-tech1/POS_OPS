...
# Lazy import — pyarrow is only needed when reading .parquet files
try:
...
def load_file(path: str) -> pd.DataFrame:
    ...
    if path.lower().endswith('.parquet'):
        if not _PYARROW_OK:
            st.error("pyarrow is not installed; cannot read Parquet files.")
            return None
...
def sales_global_overview(df):
    ...
    format_and_display(g[['SALES_CHANNEL_L1', 'NET_SALES']], numeric_cols=['NET_SALES'],
                       index_col='SALES_CHANNEL_L1', total_label='TOTAL')
...
def sales_by_channel_l2(df):
    ...
    st.plotly_chart(fig, use_container_width=True)
    format_and_display(g[['SALES_CHANNEL_L2', 'NET_SALES']], numeric_cols=['NET_SALES'],
                       index_col='SALES_CHANNEL_L2', total_label='TOTAL')
...
def sales_by_shift(df):
    ...
    st.plotly_chart(fig, use_container_width=True)
    format_and_display(g[['SHIFT', 'NET_SALES', 'PCT']], numeric_cols=['NET_SALES', 'PCT'],
                       index_col='SHIFT', total_label='TOTAL')
...
def night_vs_day_ratio(df):
    ...
    fig.add_trace(go.Bar(
        x=pivot_sorted['Night'], y=numbered_labels, orientation='h', name='Night',
        marker_color='#d62728',
        text=[f"{v:.1f}%" for v in pivot_sorted['Night']], textposition='inside'
    ))
    for i, (n_val, d_val) in enumerate(zip(pivot_sorted['Night'], pivot_sorted['Day'])):
        fig.add_annotation(x=n_val + 1, y=numbered_labels[i], text=f"{d_val:.1f}% Day",
                           showarrow=False, xanchor='left')
    fig.update_layout(title="Night vs Day Shift Sales Ratio — Stores with Night Shifts",
                      xaxis_title="% of Store Sales", height=700)
    st.plotly_chart(fig, use_container_width=True)
    table = pivot_sorted.reset_index().rename(columns={'Night': 'Night_%', 'Day': 'Day_%'})
    format_and_display(table, numeric_cols=['Night_%', 'Day_%'],
                       index_col='STORE_NAME', total_label='TOTAL')
...
def global_day_vs_night(df):
    ...
    st.plotly_chart(fig, use_container_width=True)
    format_and_display(agg, numeric_cols=['NET_SALES', 'PCT'],
                       index_col='Shift_Bucket', total_label='TOTAL')
...
def second_highest_channel_share(df):
    ...
    if missing_stores:
        add = pd.DataFrame({'STORE_NAME': list(missing_stores),
    ...
    for _, row in top_.iterrows():
        annotations.append(dict(x=row['SECOND_PCT'] + 1, y=row['STORE_NAME'],
                                text=f"{row['SECOND_CHANNEL']}", showarrow=False,
    ...
    fig.update_layout(title=f"Top {top_n} Stores by 2nd-Highest Channel Share (SALES_CHANNEL_L1)",
                      ...
                      height=max(500, 24 * len(top_)), annotations=annotations,
                      yaxis=dict(autorange='reversed'))
    st.plotly_chart(fig, use_container_width=True)
    format_and_display(second_sorted[['STORE_NAME', 'SECOND_CHANNEL', 'SECOND_PCT']],
                       numeric_cols=['SECOND_PCT'], index_col='STORE_NAME', total_label='TOTAL')
...
def bottom_30_2nd_highest(df):
    ...
    if bottom_30.empty:
        ...
        return
    fig = go.Figure()
    fig.add_trace(go.Bar(x=bottom_30['SECOND_PCT'], y=bottom_30['STORE_NAME'], orientation='h',
                         marker_color='#9aa0a6', name='Stem',
                         text=[f"{v:.1f}%" for v in bottom_30['SECOND_PCT']], textposition='outside'))
    fig.add_trace(go.Scatter(x=bottom_30['SECOND_PCT'], y=bottom_30['STORE_NAME'], mode='markers',
                             marker=dict(color='#1f77b4', size=10), name='2nd Channel %'))
    annotations = []
    for _, row in bottom_30.iterrows():
        annotations.append(dict(x=row['SECOND_PCT'] + 1, y=row['STORE_NAME'],
                                text=f"{row['SECOND_CHANNEL']}", showarrow=False,
                                xanchor='left', font=dict(size=10)))
    fig.update_layout(title="Bottom 30 Stores by 2nd-Highest Channel Share (SALES_CHANNEL_L1)",
                      xaxis_title="2nd-Highest Channel Share (% of Store NET_SALES)",
                      height=max(500, 24 * len(bottom_30)), annotations=annotations,
                      yaxis=dict(autorange='reversed'))
    st.plotly_chart(fig, use_container_width=True)
    format_and_display(bottom_30, numeric_cols=['SECOND_PCT', 'TOP_PCT'],
                       index_col='STORE_NAME', total_label='TOTAL')
...
def stores_sales_summary(df):
    ...
    sales_summary['% Contribution'] = (
        sales_summary['GROSS_SALES'] / sales_summary['GROSS_SALES'].sum() * 100).round(2)
...
def customer_traffic_storewise(df):
    ...
    if heatmap_matrix.empty:
        st.info("No customer traffic data to display.")
        return
    colorscale = [[0.0,'#E6E6E6'],[0.001,'#FFFFCC'],[0.25,'#FED976'],
                  [0.50,'#FEB24C'],[0.75,'#FD8D3C'],[1.0,'#E31A1C']]
    ...
    for i, total in enumerate(totals):
        fig.add_annotation(x=-0.6, y=i, text=f"{total:,}", showarrow=False,
                           xanchor='right', yanchor='middle', font=dict(size=11, color='black'))
    fig.add_annotation(x=-0.6, y=-1, text="<b>TOTAL</b>", showarrow=False,
                       xanchor='right', yanchor='top', font=dict(size=12, color='black'))
    fig.update_layout(title="Customer Traffic Heatmap", xaxis_title="Time of Day",
                      yaxis_title="Store Name", height=max(600, 25 * len(heatmap_matrix.index)),
    ...
    format_and_display(totals_df, numeric_cols=['Total_Receipts'],
                       index_col='STORE_NAME', total_label='TOTAL')
...
def active_tills_during_day(df):
    ...
    till_counts = (
        d.groupby(['STORE_NAME','TIME_ONLY'])['Till_Code']
          .nunique().reset_index(name='UNIQUE_TILLS')
    )
    ...
    for i, max_till in enumerate(max_vals):
        fig.add_annotation(x=-0.6, y=i, text=f"{int(max_till):,}", showarrow=False,
                           xanchor='right', yanchor='middle', font=dict(size=11, color='black'))
    fig.add_annotation(x=-0.6, y=-1, text="<b>MAX</b>", showarrow=False,
                       xanchor='right', yanchor='top', font=dict(size=12, color='black'))
    fig.update_layout(title="Peak Active Tills", xaxis_title="Time of Day", yaxis_title="Store Name",
    ...
    format_and_display(summary, numeric_cols=['MAX_ACTIVE_TILLS'],
                       index_col='STORE_NAME', total_label='TOTAL')
...
def avg_customers_per_till(df):
    ...
    colorscale = [[0.0,'#E6E6E6'],[0.001,'#e0f3db'],[0.25,'#a8ddb5'],
                  [0.50,'#43a2ca'],[0.75,'#0868ac'],[1.0,'#084081']]
    zmax = float(ratio_data.max()) if ratio_data.size else 1.0
    if zmax <= 0: zmax = 1.0
    fig = px.imshow(ratio_data, x=col_labels, y=ratio_matrix.index, text_auto=True, aspect='auto',
                    color_continuous_scale=colorscale, zmin=0, zmax=zmax,
                    labels=dict(x="Time Interval (30 min)", y="Store Name", color="Customers per Till"))
    fig.update_xaxes(side='top')
    for i, val in enumerate(max_vals):
        fig.add_annotation(x=-0.6, y=i, text=f"{val}", showarrow=False,
                           xanchor='right', yanchor='middle', font=dict(size=11, color='black'))
    fig.add_annotation(x=-0.6, y=-1, text="<b>MAX</b>", showarrow=False,
                       xanchor='right', yanchor='top', font=dict(size=12, color='black'))
    fig.update_layout(title="Customers Served per Till", xaxis_title="Time of Day",
                      yaxis_title="Store Name", height=max(600, 25 * len(ratio_matrix.index)),
                      margin=dict(l=190, r=30, t=60, b=60),
                      coloraxis_colorbar=dict(title="Customers / Till"))
    st.plotly_chart(fig, use_container_width=True)
    pivot_totals = pd.DataFrame({'STORE_NAME': ratio_matrix.index,
                                 'MAX_CUSTOMERS_PER_TILL': max_vals.astype(int)})
    format_and_display(pivot_totals, numeric_cols=['MAX_CUSTOMERS_PER_TILL'],
                       index_col='STORE_NAME', total_label='TOTAL')
...
def store_customer_traffic_storewise(df):
    ...
    def build_branch_pivot(branch_name):
        branch_df = d[d['STORE_NAME'] == branch_name]
        tmp = (branch_df.groupby(['DEPARTMENT','TIME_ONLY'])['CUST_CODE']
    ...
    colorscale = [[0.0,'#E6E6E6'],[0.001,'#e0f3db'],[0.25,'#a8ddb5'],
                  [0.50,'#43a2ca'],[0.75,'#0868ac'],[1.0,'#084081']]
    fig = px.imshow(z0, x=col_labels, y=y0, text_auto=True, aspect='auto',
                    color_continuous_scale=colorscale, zmin=0, zmax=global_zmax,
    ...
    def make_total_annotations(totals, y_labels):
        ann = []
        for i, total in enumerate(totals):
            ann.append(dict(x=-0.6, y=i, text=f"{int(total):,}", showarrow=False,
                            xanchor='right', yanchor='middle', font=dict(size=11, color='black')))
        ann.append(dict(x=-0.6, y=-1, text="<b>TOTAL</b>", showarrow=False,
                        xanchor='right', yanchor='top', font=dict(size=12, color='black')))
        return ann

    fig.update_layout(
        title=f"🕒 Customer Traffic Patterns — {init_branch} | Total Customers: {total_customers0:,}",
        xaxis_title="Time of Day", yaxis_title="Department",
        height=max(600, 25 * len(y0)), margin=dict(l=180, r=20, t=60, b=40),
    ...
    for b in branches:
        ...
        buttons.append(dict(label=b, method='update',
                            args=[{'z': [z], 'y': [y], 'x': [col_labels]},
                                  {'title': f"🕒 Customer Traffic Patterns — {b} | Total Customers: {total_customers:,}",
                                   'annotations': make_total_annotations(totals, y),
                                   'height': new_height}]))
    fig.update_layout(updatemenus=[dict(type='dropdown', x=0, xanchor='left', y=1.12, yanchor='top',
...
def cashiers_performance(df: pd.DataFrame):
    ...
    receipt_items = (
        d.groupby(['STORE_NAME', 'CUST_CODE'], as_index=False)['ITEM_CODE']
         .nunique().rename(columns={'ITEM_CODE': 'Unique_Items'})
    )
    ...
    store_summary = (
        ...
        .agg(Total_Customers=('CUST_CODE', 'nunique'),
    ...
    cashier_durations = (
        ...
        .agg(Avg_Duration_Sec=('Duration_Sec', 'mean'), Customers_Served=('CUST_CODE', 'nunique'))
    )
    ...
    _avg_items_cashier = (
        _rc_items.groupby(['STORE_NAME', 'CASHIER-COUNT'], as_index=False)['Unique_Items']
                .mean().rename(columns={'Unique_Items': 'Avg_Items_per_Receipt_Cashier'})
    )
    cashier_durations = cashier_durations.merge(_avg_items_cashier, on=['STORE_NAME', 'CASHIER-COUNT'], how='left')
    cashier_durations['Avg_Items_per_Receipt_Cashier'] = cashier_durations['Avg_Items_per_Receipt_Cashier'].round(1)
    ...
    fig = px.bar(df_branch, x='Avg_Serve_Min', y='CASHIER-COUNT', orientation='h',
                 text='Label_Text', color='Avg_Serve_Min', color_continuous_scale='Blues',
    ...
    fig.update_layout(xaxis_title="Average Serving Time (minutes)", yaxis_title="Cashier",
                      coloraxis_showscale=False, height=max(500, 25 * len(df_branch)))
    buttons = []
    for b in branches:
        ...
        buttons.append(dict(label=b, method='update',
                            args=[{'x': [dfb['Avg_Serve_Min']], 'y': [dfb['CASHIER-COUNT']],
                                   'text': [dfb['Label_Text']],
                                   'marker': {'color': dfb['Avg_Serve_Min'], 'colorscale': 'Blues'}},
                                  {'title': f"🕒 Avg Serving Time per Cashier — {b}",
                                   'height': max(500, 25 * len(dfb))}]))
    fig.update_layout(updatemenus=[dict(type='dropdown', x=0, xanchor='left', y=1.15, yanchor='top',
                                        buttons=buttons, showactive=True)])
    st.plotly_chart(fig, use_container_width=True)
    try:
        format_and_display(store_summary.reset_index(),
                           numeric_cols=['Total_Customers', 'Avg_Time_per_Customer_Min', 'Avg_Items_per_Receipt'],
                           index_col=None, total_label='TOTAL')
    except Exception:
...
def till_usage(df):
    ...
    fig = px.imshow(pivot.values, x=x, y=pivot.index,
    ...
    format_and_display(totals, numeric_cols=['Total_Receipts'],
                       index_col='Till_Code', total_label='TOTAL')
...
def tax_compliance(df):
    ...
    d['Tax_Compliant'] = np.where(
        d['CU_DEVICE_SERIAL'].replace({'nan': '', 'NaN': '', 'None': ''}).str.strip().str.len() > 0,
        'Compliant', 'Non-Compliant')
    if 'Till_Code' not in d.columns and all(c in d.columns for c in ['TILL','STORE_CODE']):
        d['Till_Code'] = d['TILL'].astype(str) + '-' + d['STORE_CODE'].astype(str)
    store_till = d.groupby(['STORE_NAME', 'Till_Code', 'Tax_Compliant'], as_index=False).agg(
        Receipts=('CUST_CODE', 'nunique'))
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=pivot.index, x=pivot['Compliant'], orientation='h',
                         name='Compliant', marker_color='#2ca02c'))
    fig.add_trace(go.Bar(y=pivot.index, x=pivot['Non-Compliant'], orientation='h',
                         name='Non-Compliant', marker_color='#d62728',
                         text=pivot['Non-Compliant'], textposition='outside'))
    fig.update_layout(barmode='stack', title=f"Tax Compliance by Till — {branch}",
                      height=max(400, 24 * len(pivot.index)))
    st.plotly_chart(fig, use_container_width=True)
    store_summary = d.groupby(['STORE_NAME', 'Tax_Compliant'], as_index=False).agg(
        Receipts=('CUST_CODE', 'nunique')).pivot(
        index='STORE_NAME', columns='Tax_Compliant', values='Receipts').fillna(0)
    store_summary['Total'] = store_summary.sum(axis=1)
    store_summary['Compliance_%'] = np.where(
        store_summary['Total'] > 0,
        (store_summary.get('Compliant', 0) / store_summary['Total'] * 100).round(1), 0.0)
    format_and_display(store_summary.reset_index(),
...
def customer_baskets_overview(df):
    ...
    global_top = (basket_count.to_frame().join(agg_data).reset_index()
                  .sort_values(metric, ascending=False).head(int(top_x)))
    global_top.insert(0, '#', range(1, len(global_top) + 1))
    st.subheader("Global Top Items")
    format_and_display(global_top.reset_index(drop=True),
                       numeric_cols=['Count_of_Baskets', 'QTY', 'NET_SALES'],
                       index_col='ITEM_NAME', total_label='TOTAL')
    ...
    branch_top = (basket_count_b.to_frame().join(agg_b).reset_index()
                  .sort_values(metric, ascending=False).head(int(top_x)))
    branch_top.insert(0, '#', range(1, len(branch_top) + 1))
    st.subheader(f"{branch} Top Items")
    format_and_display(branch_top.reset_index(drop=True),
                       numeric_cols=['Count_of_Baskets', 'QTY', 'NET_SALES'],
                       index_col='ITEM_NAME', total_label='TOTAL')
    if 'NET_SALES' in branch_df.columns:
        branch_item_sales = (branch_df.groupby('ITEM_NAME', as_index=False)['NET_SALES']
                             .sum().rename(columns={'NET_SALES': 'Branch_NET_SALES'}))
        ...
        if not zero_sales.empty:
            st.subheader(f'Items in Global Top {top_x} but Missing/Underperforming in "{branch}" (Zero Sales Only)')
            cols = ['#', 'ITEM_NAME', 'Count_of_Baskets', 'QTY', 'NET_SALES', 'Branch_NET_SALES']
            zero_sales = zero_sales[cols].reset_index(drop=True)
            format_and_display(zero_sales, numeric_cols=['Count_of_Baskets', 'QTY', 'NET_SALES', 'Branch_NET_SALES'],
                               index_col='ITEM_NAME', total_label='TOTAL')
        else:
            st.success(f'No items in Global Top {top_x} with zero sales in "{branch}".')
...
def global_category_overview_sales(df):
    ...
    fig = px.bar(g.head(20), x='NET_SALES', y='CATEGORY', orientation='h',
...
def global_category_overview_baskets(df):
    ...
    g = df.groupby('CATEGORY', as_index=False)['CUST_CODE'].nunique().rename(
        columns={'CUST_CODE': 'Baskets'}).sort_values('Baskets', ascending=False)
    format_and_display(g, numeric_cols=['Baskets'], index_col='CATEGORY', total_label='TOTAL')
    fig = px.bar(g.head(20), x='Baskets', y='CATEGORY', orientation='h',
...
def supplier_contribution(df):
    ...
    g = df.groupby('SUPPLIER_NAME', as_index=False)['NET_SALES'].sum().sort_values(
        'NET_SALES', ascending=False).head(50)
    format_and_display(g, numeric_cols=['NET_SALES'], index_col='SUPPLIER_NAME', total_label='TOTAL')
    fig = px.bar(g, x='NET_SALES', y='SUPPLIER_NAME', orientation='h',
...
def category_overview(df):
    ...
    g = df.groupby('CATEGORY', as_index=False).agg(
        Baskets=('CUST_CODE', 'nunique'), Net_Sales=('NET_SALES', 'sum')
    ).sort_values('Net_Sales', ascending=False)
    format_and_display(g, numeric_cols=['Baskets', 'Net_Sales'],
                       index_col='CATEGORY', total_label='TOTAL')
...
def branch_comparison(df):
    ...
    def top_items(branch):
        ...
        merged = (baskets.to_frame().join(totals, how='outer').fillna(0).reset_index()
                  .sort_values(metric, ascending=False).head(int(top_x)))
    ...
    fig = px.bar(combined, x=metric, y='ITEM_NAME', color='Branch', orientation='h',
                 text='Count_of_Baskets', barmode='group',
                 title=f"Branch Comparison — {a} vs {b}")
    fig.update_traces(textposition='outside')
    st.plotly_chart(fig, use_container_width=True)
    st.subheader(f"{a} Top Items")
    format_and_display(topA, numeric_cols=['Count_of_Baskets', 'QTY', 'NET_SALES'],
                       index_col='ITEM_NAME', total_label='TOTAL')
    st.subheader(f"{b} Top Items")
    format_and_display(topB, numeric_cols=['Count_of_Baskets', 'QTY', 'NET_SALES'],
                       index_col='ITEM_NAME', total_label='TOTAL')
...
def product_performance(df):
    ...
    basket_item_counts = (d.groupby(['STORE_NAME','CUST_CODE'])['ITEM_CODE'].nunique()
                          .rename('Distinct_SKUs').reset_index())
    baskets_with_item = item_data[['STORE_NAME','CUST_CODE']].drop_duplicates().assign(Has_Item=1)
    ...
    max_qty_per_basket = (item_data.groupby(['STORE_NAME','CUST_CODE'])['QTY'].sum()
                          .groupby('STORE_NAME').max())
    total_qty_per_store = item_data.groupby('STORE_NAME')['QTY'].sum()
    store_summary = (with_comp.groupby('STORE_NAME', as_index=False)
    ...
    total_row = pd.DataFrame([{
        'STORE_NAME': 'TOTAL', 'Baskets_With_Item': g_baskets,
        'Only_Item_Baskets': g_only_pct, 'With_Other_Items': g_with_pct,
        'Highest_QTY_In_Basket': int(per_store['Highest_QTY_In_Basket'].max()),
        'Total_QTY_Sold_Branch': total_qty_per_store.sum(),
        'Pct_of_Store_Customers': g_pct_store, 'Pct_of_Category_Customers': g_pct_cat,
    ...
    for col in ['Only_Item_Baskets','With_Other_Items','Pct_of_Store_Customers',
                'Pct_of_Category_Customers','Pct_of_Department_Customers']:
    ...
    chart_df = per_store[['STORE_NAME','Only_Item_Baskets','With_Other_Items']].melt(
        id_vars='STORE_NAME', var_name='Type', value_name='Percent')
    chart_df['Type'] = chart_df['Type'].map({'Only_Item_Baskets': 'Only Item (%)',
                                              'With_Other_Items': 'With Other Items (%)'})
    fig = px.bar(chart_df, x='Percent', y='STORE_NAME', color='Type', orientation='h',
                 barmode='stack', title=f"Stores — Basket Split (%) for {item_code} ({item_name})")
...
def global_loyalty_overview(df):
    ...
    receipts = (dfL.groupby(['STORE_NAME','CUST_CODE','LOYALTY_CUSTOMER_CODE'], as_index=False)
    ...
    per_branch_multi = receipts.groupby(['STORE_NAME','LOYALTY_CUSTOMER_CODE']).agg(
        Baskets_in_Store=('CUST_CODE','nunique'), Total_Value_in_Store=('Basket_Value','sum')
    ).reset_index()
    ...
    overview['Avg_Baskets_per_Customer'] = np.where(
        ...
        (overview['Total_Baskets_of_Those'] / overview['Loyal_Customers_Multi']).round(2), 0.0)
    format_and_display(overview.sort_values('Loyal_Customers_Multi', ascending=False),
                       numeric_cols=['Loyal_Customers_Multi','Total_Baskets_of_Those',
                                     'Total_Value_of_Those','Avg_Baskets_per_Customer'],
                       index_col='STORE_NAME', total_label='TOTAL')
...
def branch_loyalty_overview(df):
    ...
    d = df.copy()
    d['TRN_DATE'] = pd.to_datetime(d['TRN_DATE'], errors='coerce')
    d = d[d['LOYALTY_CUSTOMER_CODE'].replace('nan','').str.strip().astype(bool)]
    receipts = d.groupby(['STORE_NAME','CUST_CODE','LOYALTY_CUSTOMER_CODE'], as_index=False).agg(
        Basket_Value=('NET_SALES','sum'), First_Time=('TRN_DATE','min'))
    ...
    per_store = receipts[receipts['STORE_NAME'] == store].groupby(
        'LOYALTY_CUSTOMER_CODE', as_index=False).agg(
    ...
    per_store = per_store[per_store['Baskets_in_Store'] > 1].sort_values(
        ['Baskets_in_Store','Total_Value_in_Store'], ascending=False)
    format_and_display(per_store, numeric_cols=['Baskets_in_Store','Total_Value_in_Store'],
                       index_col='LOYALTY_CUSTOMER_CODE', total_label='TOTAL')
...
def customer_loyalty_overview(df):
    ...
    per_store = rc.groupby('STORE_NAME', as_index=False).agg(
        Baskets=('CUST_CODE','nunique'), Total_Value=('Basket_Value','sum'),
        First_Time=('First_Time','min'), Last_Time=('First_Time','max')
    ).sort_values(['Baskets','Total_Value'], ascending=False)
    format_and_display(per_store, numeric_cols=['Baskets','Total_Value'],
                       index_col='STORE_NAME', total_label='TOTAL')
    rc_disp = rc.copy()
    ...
    format_and_display(rc_disp[['STORE_NAME','Receipt_No','Basket_Value_KSh','First_Time']],
                       numeric_cols=['Basket_Value_KSh'], index_col='STORE_NAME', total_label='TOTAL')
...
def global_pricing_overview(df):
    ...
    grp = d.groupby(['STORE_NAME','DATE','ITEM_CODE','ITEM_NAME'], as_index=False).agg(
        ...
        Price_Min=('SP_PRE_VAT','min'), Price_Max=('SP_PRE_VAT','max'), Total_QTY=('QTY','sum'))
    ...
    summary = multi_price.groupby('STORE_NAME', as_index=False).agg(
        Items_with_MultiPrice=('ITEM_CODE','nunique'), Total_Diff_Value=('Diff_Value','sum'),
        Avg_Spread=('Price_Spread','mean'), Max_Spread=('Price_Spread','max'))
    format_and_display(summary.sort_values('Total_Diff_Value', ascending=False),
...
def branch_pricing_overview(df):
    ...
    d0['SP_PRE_VAT'] = pd.to_numeric(
        d0['SP_PRE_VAT'].astype(str).str.replace(',','',regex=False).str.strip(), errors='coerce').fillna(0.0)
    ...
    multi_sum = multi.sort_values(['DATE','Price_Spread','Total_QTY'],
    ...
    format_and_display(multi_sum[['#','DATE','ITEM_CODE','ITEM_NAME','Num_Prices',
                                   'Price_Min','Price_Max','Price_Spread','Total_QTY','Diff_Value']],
                       numeric_cols=['Num_Prices','Price_Min','Price_Max','Price_Spread','Total_QTY','Diff_Value'],
                       index_col='ITEM_CODE', total_label='TOTAL')
    price_brk = d.merge(multi[['DATE','ITEM_CODE']], on=['DATE','ITEM_CODE'], how='inner').groupby(
        ['DATE','ITEM_CODE','ITEM_NAME','SP_PRE_VAT'], as_index=False).agg(
        Qty_At_Price=('QTY','sum'), Receipts_At_Price=('CUST_CODE','nunique'),
        First_Time=('TRN_DATE','min'), Last_Time=('TRN_DATE','max'))
    price_brk = price_brk.sort_values(['DATE','ITEM_NAME','SP_PRE_VAT'],
    ...
    price_compact = price_brk[['DATE','ITEM_CODE','ITEM_NAME','SP_PRE_VAT',
                                'Qty_At_Price','Receipts_At_Price','Time_Window']].rename(
        columns={'SP_PRE_VAT':'Price','Qty_At_Price':'QTY','Receipts_At_Price':'Receipts'})
    st.subheader("Detailed — Price Breakdown (clean view)")
    format_and_display(price_compact, numeric_cols=['Price','QTY','Receipts'],
                       index_col='ITEM_CODE', total_label='TOTAL')
    ...
    receipts_detail = receipts_detail[receipt_cols].sort_values(
        ['DATE','ITEM_CODE','TRN_DATE'], ascending=[False,True,True]).reset_index(drop=True)
    ...
    st.info(f"Showing {len(receipts_detail):,} receipt rows for the affected item-days.")
    ...
    try:
        ...
        st.download_button("Download receipts as CSV", data=csv_bytes,
                           file_name=f"{branch}_multi_price_receipts.csv", mime="text/csv")
    except Exception:
        pass
...
def global_refunds_overview(df):
    ...
    d['NET_SALES'] = pd.to_numeric(
        d['NET_SALES'].astype(str).str.replace(',','',regex=False), errors='coerce').fillna(0)
    neg = d[d['NET_SALES'] < 0]
    if neg.empty:
    ...
    if 'CAP_CUSTOMER_CODE' in neg.columns:
        ...
        neg['Sale_Type'] = np.where(
            ...
            'On_account sales', 'General sales')
    else:
        neg = neg.copy()
        neg['Sale_Type'] = 'General sales'
    summary = neg.groupby(['STORE_NAME','Sale_Type'], as_index=False).agg(
        Total_Neg_Value=('NET_SALES','sum'), Total_Count=('CUST_CODE','nunique'))
    format_and_display(summary.sort_values('Total_Neg_Value'),
...
def branch_refunds_overview(df):
    ...
    agg = dfb.groupby(['STORE_NAME','CUST_CODE'], as_index=False).agg(
        Total_Value=('NET_SALES','sum'), First_Time=('TRN_DATE','min'), Cashier=('CASHIER','first'))
    format_and_display(agg.sort_values('Total_Value'), numeric_cols=['Total_Value'],
                       index_col='CUST_CODE', total_label='TOTAL')
...
def main():
    ...
    if section == "SALES":
        ...
        funcs = [sales_global_overview, sales_by_channel_l2, sales_by_shift,
                 night_vs_day_ratio, global_day_vs_night, second_highest_channel_share,
                 bottom_30_2nd_highest, stores_sales_summary]
        funcs[sales_items.index(choice)](df)
    elif section == "OPERATIONS":
        ops_items = [
            "Customer Traffic-Storewise", "Active Tills During the day",
            "Average Customers Served per Till", "Store Customer Traffic Storewise",
            "Customer Traffic-Departmentwise", "Cashiers Perfomance",
            "Till Usage", "Tax Compliance"
        ]
        choice = st.sidebar.selectbox("Operations Subsection", ops_items)
        funcs = [customer_traffic_storewise, active_tills_during_day, avg_customers_per_till,
                 store_customer_traffic_storewise, customer_traffic_departmentwise,
                 cashiers_performance, till_usage, tax_compliance]
        funcs[ops_items.index(choice)](df)
    elif section == "INSIGHTS":
        ins_items = [
            "Customer Baskets Overview", "Global Category Overview-Sales",
            "Global Category Overview-Baskets", "Supplier Contribution",
            "Category Overview", "Branch Comparison", "Product Perfomance",
            "Global Loyalty Overview", "Branch Loyalty Overview",
            "Customer Loyalty Overview", "Global Pricing Overview",
            "Branch Pricing Overview", "Global Refunds Overview", "Branch Refunds Overview"
        ]
        choice = st.sidebar.selectbox("Insights Subsection", ins_items)
        funcs = [customer_baskets_overview, global_category_overview_sales,
                 global_category_overview_baskets, supplier_contribution,
                 category_overview, branch_comparison, product_performance,
                 global_loyalty_overview, branch_loyalty_overview,
                 customer_loyalty_overview, global_pricing_overview,
                 branch_pricing_overview, global_refunds_overview, branch_refunds_overview]
        funcs[ins_items.index(choice)](df)
